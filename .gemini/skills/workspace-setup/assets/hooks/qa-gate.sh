...
if [[ "$STATE" == "$DONE_STATE_ID" ]]; then
  # Retrieve the ticket
  TICKET_JSON=$(curl -s -X GET "${KANBAN_BASE_URL}/api/v1/workspaces/${KANBAN_PROJECT_SLUG}/projects/${PROJECT_ID}/issues/${WORK_ITEM_ID}/" \
    -H "x-api-key: ${KANBAN_API_KEY}" \
  ...
  curl -s -X POST "${KANBAN_BASE_URL}/api/v1/workspaces/${KANBAN_PROJECT_SLUG}/projects/${PROJECT_ID}/issues/${WORK_ITEM_ID}/comments/" \
    -H "x-api-key: ${KANBAN_API_KEY}" \
    -H "Content-Type: application/json" \
